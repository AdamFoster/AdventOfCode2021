...
open(demo ? "day11/demoinput" : "day11/input", "r") do f # demoinput
    ...
    for i in 1:100
        ...
        #octopodes = [fill(0, size(octopodes, 1)) octopodes fill(0, size(octopodes, 1))]
        #octopodes = [fill(0, 1, size(octopodes, 2)) ; octopodes  ; fill(0, 1, size(octopodes, 2))]
        #octopodes = OffsetArray(octopodes, 0:(size(octopodes,1)-1), 0:(size(octopodes,2)-1))
    ...
    display(octopodes)
    println()
    println("Total flashes = $totalflashes") # 1642
end