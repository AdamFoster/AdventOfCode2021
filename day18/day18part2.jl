...
open(demo ? "day18/demoinput5" : "day18/input", "r") do f # demoinput
    ...
    maxmag = 0
    for c in [(x,y) for x in 1:size(ns,1), y in 1:size(ns,1)]
        if c[1] == c[2]
            #pass
        else
            total = []
            push!(total, deepcopy(ns[c[1]]))
            push!(total, deepcopy(ns[c[2]]))
            snailreduce!(total)
            mag = magnitude(total)
            println("$c => $mag from $(ns[c[1]]) & $(ns[c[2]])")
            if mag > maxmag
                maxmag = mag
                println("NEW MAX ABOVE")
            end
        end
    end
    println("Maxmag = $maxmag") # 4747
